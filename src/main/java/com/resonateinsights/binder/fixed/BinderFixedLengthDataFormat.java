 ...
 */
package com.resonateinsights.binder.fixed;

import com.resonateinsights.binder.BinderAbstractDataFormat;
import com.resonateinsights.binder.BinderAbstractFactory;
import com.resonateinsights.binder.BinderFixedLengthFactory;
import com.resonateinsights.binder.ObjectHelper;
import com.resonateinsights.binder.annotation.FixedLengthRecord;
 ...
 * {@link com.resonateinsights.binder.spi.DataFormat}) using Binder to marshal to and from Fixed Length
 */
public class BinderFixedLengthDataFormat extends BinderAbstractDataFormat {

  public static final String CAMEL_BINDY_FIXED_LENGTH_HEADER = "CamelBinderFixedLengthHeader";
  public static final String CAMEL_BINDY_FIXED_LENGTH_FOOTER = "CamelBinderFixedLengthFooter";

  private static final Logger LOG = LoggerFactory.getLogger(BinderFixedLengthDataFormat.class);

  private BinderFixedLengthFactory headerFactory;
  private BinderFixedLengthFactory footerFactory;

  public BinderFixedLengthDataFormat() {
  }

  public BinderFixedLengthDataFormat(String... packages) {
    super(packages);
  }

  public BinderFixedLengthDataFormat(Class<?> type) {
    super(type);
  }

  @SuppressWarnings("unchecked")
  public void marshal(Context context, Object body, OutputStream outputStream) throws Exception {
    PackageScanClassResolver resolver = context.getPackageScanClassResolver();
    BinderFixedLengthFactory factory = (BinderFixedLengthFactory) getFactory(resolver);
    ...
    if (context.getTypeConverter().convertTo(List.class, body) == null) {
...
//    }
  ...
  protected Map<String, Object> createModel(BinderFixedLengthFactory factory, String line, int count) throws Exception {
  ...
  protected BinderAbstractFactory createModelFactory(PackageScanClassResolver resolver) throws Exception {
    ...
    BinderFixedLengthFactory factory;
    if (getClassType() != null) {
      factory = new BinderFixedLengthFactory(resolver, defaultRecordScanFilter, getClassType());
    } else {
      factory = new BinderFixedLengthFactory(resolver, defaultRecordScanFilter, getPackages());
    }
    ...
    if (factory.hasHeader()) {
      ...
      if (getClassType() != null) {
        this.headerFactory = new BinderFixedLengthFactory(resolver, headerScanFilter, getClassType());
      } else {
        this.headerFactory = new BinderFixedLengthFactory(resolver, headerScanFilter, getPackages());
      }
    }
    ...
    if (factory.hasFooter()) {
      ...
      if (getClassType() != null) {
        this.footerFactory = new BinderFixedLengthFactory(resolver, footerScanFilter, getClassType());
      } else {
        this.footerFactory = new BinderFixedLengthFactory(resolver, footerScanFilter, getPackages());
      }
    }
